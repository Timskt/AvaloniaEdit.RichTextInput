...
全选复制或局部选择复制时，剪贴板里通常会同时存在 `DataFormat.Text` 和 `RichTextClipboardFormat`。富输入目标会优先读取 `RichTextClipboardFormat`，不会把图片、文件或业务组件按 `Image` / `report.pdf` 这类显示文本插入；普通控件才使用纯文本降级。
