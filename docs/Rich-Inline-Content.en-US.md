...
This guide documents the rich inline-content path used by `AvaloniaEdit.RichTextInput` on the `ava11-feat` branch. The same design is implemented on `ava12-feat` with the Avalonia 12 compatibility surface.
